...
link '/usr/local/bin/pg_ctl' do
  to File.join(`pg_config --bindir`.strip, 'pg_ctl')
  not_if do
    ::File.exist?('/usr/local/bin/pg_ctl')
  end
end
...
if node['repmgr']['replication']['role'] == 'master'
  ...
  unless File.exist?(File.join(node['postgresql']['config']['data_directory'], 'recovery.conf'))
    ...
    ruby_block 'confirm slave status' do
      block do
        ...
        Chef::Log.fatal "OUTPUT: #{`sudo -u postgres repmgr -f #{node['repmgr']['config_file_path']} cluster show`}"
        recovery_file = File.join(node['postgresql']['config']['data_directory'], 'recovery.conf')
        if File.exist?(recovery_file)

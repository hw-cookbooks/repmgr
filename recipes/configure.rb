...
if node['repmgr']['replication']['role'] == 'master'
  unless node['repmgr']['replication']['user_password']
    ...
    node.normal['repmgr']['replication'][:user_password] = pg_pass
...
if node['repmgr']['replication']['role'] == 'master'
  ...
  # Configurations!
  node.normal['postgresql']['config'][:wal_level] = 'hot_standby'
  node.normal['postgresql']['config'][:archive_mode] = true
  node.normal['postgresql']['config'][:listen_addresses] = node['repmgr']['replication']['listen_addresses']
  node.normal['postgresql']['config'][:hot_standby] = true

  node.normal['repmgr']['addressing'][:master] = node['repmgr']['addressing']['self']

else
  node.normal['repmgr'][:replication_role] = 'slave'
  node.normal['postgresql']['config'][:hot_standby] = node['repmgr']['readonly_slave']
  node.normal['postgresql']['config'][:wal_level] = 'hot_standby'
  node.normal['postgresql']['config'][:hot_standby_feedback] = node['repmgr']['replication']['standby_feedback']
  node.normal['postgresql']['config'][:max_standby_streaming_delay] = node['repmgr']['replication']['max_streaming_delay']
  ...
  if master_node
    ...
    file '/var/lib/postgresql/.ssh/known_hosts' do
      content `ssh-keyscan #{node['repmgr']['addressing']['master']}`
    end
  end
end

node.normal['postgresql']['config'][:archive_command] = node['repmgr']['replication']['archive_command']
node.normal['postgresql']['config'][:archive_timeout] = node['repmgr']['replication']['archive_timeout']
node.normal['postgresql']['config'][:max_wal_senders] = node['repmgr']['replication']['max_senders']
node.normal['postgresql']['config'][:wal_keep_segments] = node['repmgr']['replication']['keep_segments']

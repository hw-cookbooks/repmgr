...
node.default['repmgr'][:pg_bin_dir] = `pg_config --bindir`.strip
...
when 'debian'
  execute 'add repmgr to default paths' do
    ...
    not_if do
      `update-alternatives --display repmgr`.split("\n").last.to_s.strip.split(' ').last.to_s.gsub(/('|\.$)/, '') == File.join(node['repmgr']['pg_bin_dir'], 'repmgr')
    end
  end
  execute 'add repmgrd to default paths' do
    ...
    not_if do
      `update-alternatives --display repmgrd`.split("\n").last.to_s.strip.split(' ').last.to_s.gsub(/('|\.$)/, '') == File.join(node['repmgr']['pg_bin_dir'], 'repmgrd')
    end
  end
when 'rhel'
  # TODO
...
when 'upstart'
  ...
  template '/etc/init.d/repmgrd' do
    ...
    if File.exist?('/etc/init.d/repmgrd')
